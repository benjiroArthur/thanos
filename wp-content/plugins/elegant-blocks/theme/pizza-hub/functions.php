...
function bizberg_after_import( $selected_import ) {
	...
	switch ( $import_file_name ) {
		...
		case 'Simple Homepage Free':			
			$args['default_page']['page_on_front'] = 'restaurant-homepage-free';
		...
		case 'Homepage FREE':			
			...
			bizberg_set_page_theme_mod('homepage-free');
			break;

		case 'Homepage ( Video Banner )':			
...
function bizberg_set_defaults( $args ){
	...
	$args['search_replace'][] = 'restaurant-homepage-free';
...
function bizberg_search_replace_content( $slug ){
	...
	switch ( $slug ) {
		...
		case 'restaurant-homepage-free':
			...
			// Get Gallery Term ID
			$gallery_cat_id = bizberg_get_taxonomy_id_by_slug( $cat_name = 'gallery' , $tax = 'ct_gallery_category' );
			$category_new = '"selectedGalleryCategoryID":"' . $gallery_cat_id . '"';
			$content = str_replace( '"selectedGalleryCategoryID":"57"' , $category_new , $content );
			...
			$content = str_replace( '"selectedCategoryID":49' , $category_new , $content );
			...
			$content = str_replace( '"selectedTestimonialCategoryID":"53"' , $category_new , $content );
			...
			$content = str_replace( '"selectedClientCatID":"38"' , $category_new , $content );
		...
		case 'restaurant-homepage':
			...
			$slider_cat_id = bizberg_get_taxonomy_id_by_slug( $cat_name = 'restaurant' , $tax = 'ct_slider_category' );
			$category_new = '"selectedSliderCategoryID":"' . $slider_cat_id . '"';
			$content = str_replace( '"selectedSliderCategoryID":"38"' , $category_new , $content );

			// Get Gallery Term ID
			$gallery_cat_id = bizberg_get_taxonomy_id_by_slug( $cat_name = 'gallery' , $tax = 'ct_gallery_category' );
			$category_new = '"selectedGalleryCategoryID":"' . $gallery_cat_id . '"';
			$content = str_replace( '"selectedGalleryCategoryID":"36"' , $category_new , $content );

			// Get Team Term ID
			$team_cat_id = bizberg_get_taxonomy_id_by_slug( $cat_name = 'our-chefs' , $tax = 'ct_team_category' );
			$category_new = '"selectedCategoryID":' . $team_cat_id;
			$content = str_replace( '"selectedCategoryID":37' , $category_new , $content );

			// Get Testimonial Term ID
			$testimonials_cat_id = bizberg_get_taxonomy_id_by_slug( $cat_name = 'testimonial' , $tax = 'ct_testimonial_category' );
			$category_new = '"selectedTestimonialCategoryID":"' . $testimonials_cat_id . '"';
			$content = str_replace( '"selectedTestimonialCategoryID":"39"' , $category_new , $content );

			// Get Client Term ID
			$client_cat_id = bizberg_get_taxonomy_id_by_slug( $cat_name = 'clients' , $tax = 'client_cat' );
			$category_new = '"selectedClientCatID":"' . $client_cat_id . '"';
			$content = str_replace( '"selectedClientCatID":"35"' , $category_new , $content );
