...
function bizberg_search_replace_content( $slug ){
	...
	switch ( $slug ) {
		...
		case 'nature-free':
			...
			// Get accordion
			$post_cat_id = bizberg_get_taxonomy_id_by_slug( $cat_name = 'our-recent-projects' , $tax = 'category' );
			$category_new = '"selectedPostCategoryID":"' . $post_cat_id . '"';
			$content = str_replace( '"selectedPostCategoryID":"4"' , $category_new , $content );
