...
function bizberg_after_import( $selected_import ) {
	...
	switch ( $import_file_name ) {
		...
		case 'Building Construction Architecture Free':			
...
function bizberg_search_replace_content( $slug ){
	...
	switch ( $slug ) {
		...
		case 'construction-free':
			...
			$content = str_replace( '"selectedSliderCategoryID":"57"' , $category_new , $content );
			...
			// Get Blog
			$courses_cat_id = bizberg_get_taxonomy_id_by_slug( $cat_name = 'our-services' , $tax = 'category' );
			$category_new = '"categories":"' . $courses_cat_id . '"';
			$content = str_replace( '"categories":"10"' , $category_new , $content );
