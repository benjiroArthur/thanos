...
function bizberg_after_import( $selected_import ) {
	...
	switch ( $import_file_name ) {
		...
		case 'NGO Charity Fundraising Free':			
...
function bizberg_search_replace_content( $slug ){
	...
	switch ( $slug ) {
		...
		case 'charity-free':
			...
			// Get Courses
			$courses_cat_id = bizberg_get_taxonomy_id_by_slug( $cat_name = 'our-causes' , $tax = 'category' );
			...
			$content = str_replace( '"selectedTestimonialCategoryID":"6"' , $category_new , $content );
			...
			$content = str_replace( '"selectedCategoryID":7' , $category_new , $content );
