...
function bizberg_after_import( $selected_import ) {
	...
	switch ( $import_file_name ) {
		...
		case 'Simple Homepage Free':			
			$args['default_page']['page_on_front'] = 'clinic-free';
			bizberg_set_defaults( $args );
			break;

		case 'Homepage 1':			
			$args['default_page']['page_on_front'] = 'homepage-1';
			bizberg_set_defaults( $args );
			break;

		case 'Homepage Transparent Header':			
			$args['default_page']['page_on_front'] = 'transparent-header';
			bizberg_set_defaults( $args );
			break;

		case 'Homepage Slider 1':			
			$args['default_page']['page_on_front'] = 'homepage-slider';
			bizberg_set_defaults( $args );
			break;
...
function bizberg_set_team_meta_data(){
	...
	$terms = array( 'team' );
	$chefs = array( 'Rehabilitation Therapy' , 'Cardiologist' , 'Throat Specialist' , 'MBBS Teacher' );
...
function bizberg_set_defaults( $args ){
	...
	$args['search_replace'][] = 'clinic-free';
	$args['search_replace'][] = 'homepage-1';
	$args['search_replace'][] = 'transparent-header';
	$args['search_replace'][] = 'homepage-slider';
	$args['search_replace'][] = 'homepage-slider-2';
	$args['search_replace'][] = 'homepage-slider-3';
	$args['search_replace'][] = 'about-us';
	$args['search_replace'][] = 'pricing';
	$args['search_replace'][] = 'gallery-1';
...
function bizberg_search_replace_content( $slug ){
	...
	switch ( $slug ) {
		...
		case 'clinic-free':
			...
			$contact_form_7_id = bizberg_set_contact_form_7_id( 'Medical Form 1' );
			...
			$content = str_replace( '"selectedCategoryID":5' , $category_new , $content );

			// Get Client Term ID
			$client_cat_id = bizberg_get_taxonomy_id_by_slug( $cat_name = 'partners' , $tax = 'client_cat' );
			$category_new = '"selectedClientCatID":"' . $client_cat_id . '"';
			$content = str_replace( '"selectedClientCatID":"4"' , $category_new , $content );
		...
		case 'homepage-slider-3':
			...
			$content = str_replace( '"selectedSliderCategoryID":"21"' , $category_new , $content );
			...
			$category_new = '"selectedCategoryID":' . $team_cat_id;
			$content = str_replace( '"selectedCategoryID":13' , $category_new , $content );

			// Get Client Term ID
			$client_cat_id = bizberg_get_taxonomy_id_by_slug( $cat_name = 'partners' , $tax = 'client_cat' );
			$category_new = '"selectedClientCatID":"' . $client_cat_id . '"';
			$content = str_replace( '"selectedClientCatID":"12"' , $category_new , $content );
		...
		case 'gallery-1':
			...
			$content = str_replace( '"selectedGalleryCategoryID":"23"' , $category_new , $content );
